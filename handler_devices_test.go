...
func setupDevicesTestDB(t *testing.T) *sql.DB {
...
func TestHandleListDevices(t *testing.T) {
	...
	createTestDevice(t, db, "DEV002", "IPN-200", "v1.1.0", "Widget Inc", "Lab 3", "inactive", "2024-02-20", "")

	req := httptest.NewRequest("GET", "/api/v1/devices", nil)
	w := httptest.NewRecorder()

	handleListDevices(w, req)
...
func TestHandleGetDevice_NotFound(t *testing.T) {
	...
	handleGetDevice(w, req, "NONEXISTENT")
...
func TestHandleCreateDevice(t *testing.T) {
	...
	var count int
	db.QueryRow("SELECT COUNT(*) FROM devices WHERE serial_number = ?", "DEV001").Scan(&count)
	if count != 1 {
...
func TestHandleDeviceHistory(t *testing.T) {
	...
	// Add test record
	now := time.Now().Format("2006-01-02 15:04:05")
	db.Exec(`INSERT INTO test_records (serial_number, ipn, firmware_version, test_type, result, measurements, notes, tested_by, tested_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		"DEV001", "IPN-100", "v1.0.0", "functional", "pass", "{}", "All tests passed", "testuser", now)

	// Add campaign device
	db.Exec(`INSERT INTO campaign_devices (campaign_id, serial_number, status, updated_at)
		VALUES (?, ?, ?, ?)`,
