...
func setupFieldReportsTestDB(t *testing.T) func() {
...
func TestFieldReportUpdateNotFound(t *testing.T) {
	...
	w := httptest.NewRecorder()
	handleUpdateFieldReport(w, req, "FR-9999")
	if w.Code != 404 {
		t.Errorf("expected 404, got %d", w.Code)
	}
}
...
func TestFieldReportDeleteNotFound(t *testing.T) {
	...
	w := httptest.NewRecorder()
	handleDeleteFieldReport(w, req, "FR-9999")
	if w.Code != 404 {
		t.Errorf("expected 404, got %d", w.Code)
	}
}
...
func TestFieldReportDateRangeFilter(t *testing.T) {
	...
	db.Exec(`INSERT INTO field_reports (id, title, created_at) VALUES 
...
// Helper functions
// Note: stringReader is defined in handler_changes_test.go
