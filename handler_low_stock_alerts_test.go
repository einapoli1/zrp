...
func TestLowStockAlerts_NoAlert_WhenAboveThreshold(t *testing.T) {
	oldDB := db
	db = setupLowStockTestDB(t)
	defer func() { db.Close(); db = oldDB }()

	// Insert part with qty above reorder point
	_, err := db.Exec(`INSERT INTO inventory (ipn, qty_on_hand, reorder_point, description) VALUES 
...
func TestLowStockAlerts_AlertClears_WhenReplenished(t *testing.T) {
	...
	handleInventoryTransact(wTransact, reqTransact)
	...
	db.QueryRow("SELECT qty_on_hand FROM inventory WHERE ipn=?", "PART-003").Scan(&qtyOnHand)
	if qtyOnHand != 15 {
...
func TestLowStockAlerts_Sequential_ReplenishAndDeplete(t *testing.T) {
	...
	db.Exec(`INSERT INTO inventory (ipn, qty_on_hand, reorder_point) VALUES ('PART-SEQ', 5, 10)`)
	...
	handleInventoryTransact(wT1, reqT1)
