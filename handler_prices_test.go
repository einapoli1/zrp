...
func insertTestVendor(t *testing.T, db *sql.DB, id, name string) {
	_, err := db.Exec(
...
func TestHandleListPrices_Empty(t *testing.T) {
	...
	handleListPrices(w, req, "IPN-001")
...
// Test handleListPrices - With Data
func TestHandleListPrices_WithData(t *testing.T) {
	oldDB := db
	db = setupPricesTestDB(t)
	defer func() { db.Close(); db = oldDB }()

	insertTestVendor(t, db, "V-001", "Acme Corp")
	insertTestVendor(t, db, "V-002", "Beta Inc")
...
// Test handleCreatePrice - Success with vendor_id
...
// Test handleCreatePrice - Success with vendor_name
...
// Test handleCreatePrice - Default values
func TestHandleCreatePrice_Defaults(t *testing.T) {
	...
	price := map[string]interface{}{
		"ipn":        "IPN-DEFAULTS",
		"unit_price": 99.99,
		// No currency or min_qty specified
	}
...
// Test handleCreatePrice - Validation errors
...
// Test handleDeletePrice - Success
...
// Test handleDeletePrice - Not Found
...
func TestHandlePriceTrend_Empty(t *testing.T) {
	...
	handlePriceTrend(w, req, "IPN-001")
...
// Test handlePriceTrend - With Data
func TestHandlePriceTrend_WithData(t *testing.T) {
	oldDB := db
	db = setupPricesTestDB(t)
	defer func() { db.Close(); db = oldDB }()

	insertTestVendor(t, db, "V-001", "Vendor A")
	insertTestVendor(t, db, "V-002", "Vendor B")
	...
	handlePriceTrend(w, req, "IPN-TREND")
...
// Test handlePriceTrend - Multiple currencies
func TestHandlePriceTrend_MultipleCurrencies(t *testing.T) {
	oldDB := db
	db = setupPricesTestDB(t)
	defer func() { db.Close(); db = oldDB }()

	insertTestVendor(t, db, "V-001", "US Vendor")
	insertTestVendor(t, db, "V-002", "EU Vendor")
	...
	handlePriceTrend(w, req, "IPN-MULTI")
...
// Test recordPriceFromPO
func TestRecordPriceFromPO(t *testing.T) {
	oldDB := db
	db = setupPricesTestDB(t)
	defer func() { db.Close(); db = oldDB }()

	insertTestVendor(t, db, "V-001", "PO Vendor")

	// Record price from PO
	recordPriceFromPO("PO-001", "IPN-PO", 25.50, "V-001")

	// Verify price was recorded
	var count int
	db.QueryRow("SELECT COUNT(*) FROM price_history WHERE ipn=? AND po_id=?", "IPN-PO", "PO-001").Scan(&count)
	if count != 1 {
		t.Errorf("Expected 1 price record from PO, got %d", count)
	}

	var unitPrice float64
	var vendorName string
	db.QueryRow("SELECT unit_price, vendor_name FROM price_history WHERE ipn=? AND po_id=?", "IPN-PO", "PO-001").Scan(&unitPrice, &vendorName)
	
	if unitPrice != 25.50 {
		t.Errorf("Expected unit_price 25.50, got %.2f", unitPrice)
	}
	if vendorName != "PO Vendor" {
		t.Errorf("Expected vendor_name 'PO Vendor', got %s", vendorName)
	}
}

// Test recordPriceFromPO - Zero or negative price (should skip)
func TestRecordPriceFromPO_InvalidPrice(t *testing.T) {
	oldDB := db
	db = setupPricesTestDB(t)
	defer func() { db.Close(); db = oldDB }()

	insertTestVendor(t, db, "V-001", "Test Vendor")

	// Try to record with zero price
	recordPriceFromPO("PO-001", "IPN-ZERO", 0, "V-001")

	// Verify nothing was recorded
	var count int
	db.QueryRow("SELECT COUNT(*) FROM price_history WHERE ipn=?", "IPN-ZERO").Scan(&count)
	if count != 0 {
		t.Errorf("Expected 0 price records for zero price, got %d", count)
	}

	// Try to record with negative price
	recordPriceFromPO("PO-002", "IPN-NEG", -10.0, "V-001")

	// Verify nothing was recorded
	db.QueryRow("SELECT COUNT(*) FROM price_history WHERE ipn=?", "IPN-NEG").Scan(&count)
	if count != 0 {
		t.Errorf("Expected 0 price records for negative price, got %d", count)
	}
}
...
func TestPriceHistory_ChronologicalOrdering(t *testing.T) {
	...
	insertTestVendor(t, db, "V-001", "Vendor")
	...
	handleListPrices(w, req, "IPN-ORDER")
