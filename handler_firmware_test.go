...
func setupFirmwareTestDB(t *testing.T) *sql.DB {
...
func insertTestDevice(t *testing.T, db *sql.DB, serial, ipn, status string) {
	_, err := db.Exec(
...
func TestHandleListCampaigns_WithData(t *testing.T) {
	...
	insertTestCampaign(t, db, "FW-001", "Campaign 1", "v1.0.0", "public", "draft")
	insertTestCampaign(t, db, "FW-002", "Campaign 2", "v1.1.0", "public", "active")
	insertTestCampaign(t, db, "FW-003", "Campaign 3", "v2.0.0", "beta", "completed")
...
func TestHandleGetCampaign_Success(t *testing.T) {
	...
	handleGetCampaign(w, req, "FW-001")
...
func TestHandleGetCampaign_NotFound(t *testing.T) {
	...
	handleGetCampaign(w, req, "FW-999")
...
func TestHandleCreateCampaign_Success(t *testing.T) {
	...
	var count int
	db.QueryRow("SELECT COUNT(*) FROM firmware_campaigns WHERE name=?", "New Campaign").Scan(&count)
	if count != 1 {
...
func TestHandleUpdateCampaign_Success(t *testing.T) {
	...
	insertTestCampaign(t, db, "FW-001", "Original", "v1.0.0", "public", "draft")
	...
	db.QueryRow("SELECT name, version FROM firmware_campaigns WHERE id=?", "FW-001").Scan(&name, &version)
...
func TestHandleLaunchCampaign_Success(t *testing.T) {
	...
	insertTestCampaign(t, db, "FW-001", "Launch Test", "v1.0.0", "public", "draft")
	insertTestDevice(t, db, "SN-001", "IPN-001", "active")
	insertTestDevice(t, db, "SN-002", "IPN-001", "active")
	insertTestDevice(t, db, "SN-003", "IPN-002", "inactive")
	...
	var count int
	db.QueryRow("SELECT COUNT(*) FROM campaign_devices WHERE campaign_id=?", "FW-001").Scan(&count)
	if count != 2 {
...
func TestHandleMarkCampaignDevice_Success(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Reset device status
			db.Exec("UPDATE campaign_devices SET status='pending' WHERE campaign_id=? AND serial_number=?", "FW-001", "SN-001")
			...
			var status string
			db.QueryRow("SELECT status FROM campaign_devices WHERE campaign_id=? AND serial_number=?", "FW-001", "SN-001").Scan(&status)
			if status != tt.status {
...
func TestHandleMarkCampaignDevice_NotFound(t *testing.T) {
	...
	handleMarkCampaignDevice(w, req, "FW-001", "SN-999")
...
func TestHandleCampaignDevices(t *testing.T) {
	...
	handleCampaignDevices(w, req, "FW-001")
...
func TestHandleCampaignDevices_Empty(t *testing.T) {
	...
	insertTestCampaign(t, db, "FW-001", "Empty Devices Test", "v1.0.0", "public", "draft")
