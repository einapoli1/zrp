...
func TestSerialNumberAutoGeneration(t *testing.T) {
	...
	handleWorkOrderAddSerial(rr, req, "WO001")
...
func TestSerialTraceability(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handleWorkOrderSerials(rr, req, "WO002")
...
func TestReverseSerialTraceability(t *testing.T) {
	...
	for _, wo := range workOrders {
		...
		for _, sn := range wo.serials {
			_, err := db.Exec(`INSERT INTO wo_serials (wo_id, serial_number, status) 
...
func TestDuplicateSerialNumberRejection(t *testing.T) {
	...
	handleWorkOrderAddSerial(rr1, req1, "WO006")
	...
	handleWorkOrderAddSerial(rr2, req2, "WO007")
...
func TestSerialStatusTransitions(t *testing.T) {
	...
	for _, newStatus := range validTransitions {
		_, err := db.Exec(`UPDATE wo_serials SET status = ? WHERE serial_number = 'SN-STATUS-001'`, newStatus)
		if err != nil {
			t.Errorf("Failed to transition to status %s: %v", newStatus, err)
		}

		var currentStatus string
		db.QueryRow(`SELECT status FROM wo_serials WHERE serial_number = 'SN-STATUS-001'`).Scan(&currentStatus)
	...
	_, err = db.Exec(`UPDATE wo_serials SET status = 'invalid_status' WHERE serial_number = 'SN-STATUS-001'`)
	if err == nil {
...
func TestSerialWorkOrderCascadeDelete(t *testing.T) {
	...
	var count int
	db.QueryRow(`SELECT COUNT(*) FROM wo_serials WHERE wo_id = 'WO009'`).Scan(&count)
	if count != 2 {
		t.Errorf("Expected 2 serials before delete, got %d", count)
	}

	// Delete work order
	_, err = db.Exec(`DELETE FROM work_orders WHERE id = 'WO009'`)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWorkOrderCompletionWithSerials(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		...
		rr := httptest.NewRecorder()
		handleWorkOrderAddSerial(rr, req, "WO012")
	...
	var serialCount int
	db.QueryRow(`SELECT COUNT(*) FROM wo_serials WHERE wo_id = 'WO012'`).Scan(&serialCount)
	if serialCount != 3 {
	...
	// Get serial IDs
	rows, err := db.Query(`SELECT id FROM wo_serials WHERE wo_id = 'WO012' ORDER BY id`)
	if err != nil {
		t.Fatal(err)
	}
	var serialIDs []int
	...
	db.Exec(`UPDATE wo_serials SET status = 'complete' WHERE id = ?`, serialIDs[0])
	db.Exec(`UPDATE wo_serials SET status = 'complete' WHERE id = ?`, serialIDs[1])
	// Mark third as failed
	db.Exec(`UPDATE wo_serials SET status = 'failed' WHERE id = ?`, serialIDs[2])

	// Count good vs failed
	var goodCount, failedCount int
	db.QueryRow(`SELECT COUNT(*) FROM wo_serials WHERE wo_id = 'WO012' AND status = 'complete'`).Scan(&goodCount)
	db.QueryRow(`SELECT COUNT(*) FROM wo_serials WHERE wo_id = 'WO012' AND status = 'failed'`).Scan(&failedCount)
	...
	_, err = db.Exec(`UPDATE work_orders SET qty_good = ?, qty_scrap = ?, status = 'complete', 
	...
	err = db.QueryRow(`SELECT qty_good, qty_scrap FROM work_orders WHERE id = 'WO012'`).Scan(&qtyGood, &qtyScrap)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSerialSearchAndLookup(t *testing.T) {
	...
	err = db.QueryRow(`SELECT serial_number FROM wo_serials WHERE serial_number LIKE ?`, 
	...
	err = db.QueryRow(`SELECT COUNT(*) FROM wo_serials WHERE status = 'building'`).Scan(&buildingCount)
	if err != nil {
		t.Fatal(err)
	}
