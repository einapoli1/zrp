...
func pageNCRsList(w http.ResponseWriter, r *http.Request) {
	...
	status := r.URL.Query().Get("status")
	query := "SELECT id,title,COALESCE(description,''),COALESCE(ipn,''),COALESCE(serial_number,''),COALESCE(defect_type,''),severity,status,COALESCE(root_cause,''),COALESCE(corrective_action,''),created_at,resolved_at FROM ncrs"
	var args []interface{}
	if status != "" {
		query += " WHERE status=?"
		args = append(args, status)
	...
	pageFiles := []string{"templates/ncr/list.html"}
	if isHTMX(r) {
		renderPartial(w, pageFiles, "ncrs-table", data)
		return
	}
