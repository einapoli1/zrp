...
func handleCalendar(w http.ResponseWriter, r *http.Request) {
	...
	if monthStr != "" {
		if m, err := strconv.Atoi(monthStr); err == nil { month = m }
	}

	startDate := fmt.Sprintf("%04d-%02d-01", year, month)
	endDate := fmt.Sprintf("%04d-%02d-31", year, month) // 31 is fine, DB handles it

	var events []CalendarEvent

	// Work Orders: use completed_at target, or created_at + 30 days
	...
	if err == nil {
		...
		for rows.Next() {
			...
			if len(dueDate) >= 10 { dueDate = dueDate[:10] }
	...
	if err == nil {
		...
		for rows2.Next() {
			...
			if len(expDate) >= 10 { expDate = expDate[:10] }
	...
	if err == nil {
		...
		for rows3.Next() {
			...
			if len(validUntil) >= 10 { validUntil = validUntil[:10] }
			title := fmt.Sprintf("Quote %s expires", id)
			if customer != "" { title = fmt.Sprintf("Quote for %s expires", customer) }
			events = append(events, CalendarEvent{Date: validUntil, Type: "quote", ID: id, Title: title, Color: "orange"})
		}
	}

	if events == nil { events = []CalendarEvent{} }
	jsonResp(w, events)
}
