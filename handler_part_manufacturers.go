...
// Migration function to move existing manufacturer/mpn data from parts table
func migrateExistingManufacturers(database *sql.DB) error {
	// Verify part_manufacturers table exists
	var exists int
	err := database.QueryRow(`SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='part_manufacturers'`).Scan(&exists)
	if err != nil || exists == 0 {
		// Table doesn't exist yet, skip migration
		return nil
	}

	// Query parts with manufacturer and mpn populated
	rows, err := database.Query(`
		SELECT ipn, manufacturer, mpn 
		FROM parts 
		WHERE manufacturer != '' AND mpn != ''
		AND ipn NOT IN (SELECT DISTINCT part_id FROM part_manufacturers)
	`)
	if err != nil {
		return fmt.Errorf("failed to query parts: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ipn, &manufacturer, &mpn); err != nil {
			return fmt.Errorf("failed to scan row: %w", err)
		}

		// Insert as primary manufacturer
		_, err = tx.Exec(`
			INSERT INTO part_manufacturers (part_id, manufacturer, mpn, is_primary, approved, notes, created_at, updated_at)
			VALUES (?, ?, ?, 1, 1, 'Migrated from parts table', ?, ?)
		`, ipn, manufacturer, mpn, time.Now().Format(time.RFC3339), time.Now().Format(time.RFC3339))
		if err != nil {
			// Skip duplicates (shouldn't happen due to NOT IN clause, but be safe)
			if !strings.Contains(err.Error(), "UNIQUE constraint failed") {
				return fmt.Errorf("failed to insert manufacturer for %s: %w", ipn, err)
			}
		} else {
			count++
		}
	...
	if count > 0 {
		fmt.Printf("Migrated %d manufacturers from parts table\n", count)
	}
