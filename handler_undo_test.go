...
func setupUndoTestDB(t *testing.T) *sql.DB {
	...
	testDB.Exec(`CREATE TABLE ecos (id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT, type TEXT DEFAULT 'change', status TEXT DEFAULT 'draft' CHECK(status IN ('draft','review','approved','implemented','rejected','cancelled','pending')), priority TEXT DEFAULT 'normal', affected_ipns TEXT DEFAULT '[]', created_by TEXT DEFAULT '', created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, approved_at DATETIME, approved_by TEXT)`)
