...
// execPermissionTest executes a permission test via requireRBAC middleware
func execPermissionTest(t *testing.T, method, path, role string, body interface{}, expectedStatus int) {
	t.Helper()
	
	// Create a success handler that simulates actual API behavior
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// If we reach here, RBAC permission was granted
		// Simulate realistic responses based on HTTP method
		if method == "DELETE" || method == "PUT" {
			// Simulate resource not found for delete/update operations
	...
	rbacHandler := requireRBAC(handler)
	
	req := makeAuthRequest(method, path, role, body)
	w := httptest.NewRecorder()
	rbacHandler.ServeHTTP(w, req)
...
func TestPermissionEnforcement_ECOApprove(t *testing.T) {
	...
	t.Run("User role with eco:approve can approve ECOs", func(t *testing.T) {
		execPermissionTest(t, "POST", "/api/v1/ecos/1/approve", "user", nil, http.StatusOK) // Permission granted - returns 200
	})

	t.Run("Admin role can approve ECOs", func(t *testing.T) {
		execPermissionTest(t, "POST", "/api/v1/ecos/1/approve", "admin", nil, http.StatusOK)
	})
...
// TestPermissionEnforcement_ECOImplement tests eco:approve permission for implement action
func TestPermissionEnforcement_ECOImplement(t *testing.T) {
	...
	t.Run("Admin can implement ECOs", func(t *testing.T) {
		execPermissionTest(t, "POST", "/api/v1/ecos/1/implement", "admin", nil, http.StatusOK)
	})
}
...
func TestPermissionEnforcement_AdminFullAccess(t *testing.T) {
	...
	adminTests := []struct {
		...
		{"Create Part", "POST", "/api/v1/parts", map[string]string{"part_number": "ADMIN-001"}},
		{"View Parts", "GET", "/api/v1/parts", nil},
		{"Update Part", "PUT", "/api/v1/parts/999", map[string]string{"name": "Updated"}},
		{"Delete Part", "DELETE", "/api/v1/parts/999", nil},

		// ECOs
		{"Create ECO", "POST", "/api/v1/ecos", map[string]string{"title": "Admin ECO"}},
		{"View ECOs", "GET", "/api/v1/ecos", nil},
		{"Approve ECO", "POST", "/api/v1/ecos/999/approve", nil},
		{"Implement ECO", "POST", "/api/v1/ecos/999/implement", nil},

		// Documents
		{"Create Doc", "POST", "/api/v1/docs", map[string]string{"title": "Admin Doc"}},
		{"Approve Doc", "POST", "/api/v1/docs/999/approve", nil},

		// Vendors
		{"Create Vendor", "POST", "/api/v1/vendors", map[string]string{"name": "Admin Vendor"}},
		{"Delete Vendor", "DELETE", "/api/v1/vendors/999", nil},

		// Admin-only endpoints
		{"View Users", "GET", "/api/v1/users", nil},
		{"Create User", "POST", "/api/v1/users", map[string]interface{}{"username": "test", "password": "Pass123!"}},
		{"View API Keys", "GET", "/api/v1/apikeys", nil},
		{"Create API Key", "POST", "/api/v1/apikeys", map[string]string{"name": "Test Key"}},
	}
	...
	for _, tt := range adminTests {
		t.Run("Admin can "+tt.name, func(t *testing.T) {
			...
			w := httptest.NewRecorder()
			rbac := requireRBAC(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				w.WriteHeader(http.StatusOK)
			}))
...
// TestPermissionEnforcement_UserRoleRestrictedFromAdmin tests user role cannot access admin endpoints
...
func TestPermissionEnforcement_CustomRole(t *testing.T) {
	...
	t.Run("parts_editor can create parts", func(t *testing.T) {
		execPermissionTest(t, "POST", "/api/v1/parts", "parts_editor",
			map[string]string{"part_number": "PE-001"}, http.StatusOK)
	})
...
// TestPermissionEnforcement_CategoryManagement tests parts category permissions (part of parts module)
